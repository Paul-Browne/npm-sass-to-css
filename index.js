...
const processCss = async (css, inPath, outPath, maps) => {
	...
	if(result.css.indexOf("\n/*# source") !== 0){
		await makeFile(outPath, result.css);
	}
}

const resultToMapAndCss = async (err, result, inPath, outPath, maps) => {
	if (err) throw err;
	if(maps && result.map && JSON.parse(result.map).mappings){
		await makeFile(outPath + ".map", result.map);
	} 
    await processCss(result.css, inPath, outPath, maps);
}
...
export default async (inPath, outPath, options) => {
	...
	if(type(inPath) === 'less'){
		renderLess(contents, { sourceMap: {} }, async (err, result) => {
		    await resultToMapAndCss(err, result, inPath, outPath, maps);
		})
		...
		await processCss(contents, inPath, outPath, false);
	}
};
